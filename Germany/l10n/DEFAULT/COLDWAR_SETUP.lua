...
zones.mendig:addGroups({
	...
	GroupCommander:new({name='Mendig-attack-Hahn-Arty', mission='attack', targetzone='Hahn', type='surface', SetActiveMission = true}),
...
zones.bornholm:addGroups({
    GroupCommander:new({name='bornholm-supply-Bornholm', mission='supply',template='PlaneSupplyTemplate', targetzone='Bornholm'}),
    GroupCommander:new({name='bornholm-supply-Peenemunde', mission='supply',template='PlaneSupplyTemplate', targetzone='Peenemunde'}),
...
function checkAndGenerateReconMissionV2()
	...
	for _, connection in ipairs(bc.connections or {}) do
		...
		if from and to and from.side ~= to.side and from.side ~= 0 and to.side ~= 0 and
			((not to.suspended) or from.suspended) then
			if checkValid(from) and not seen[from.zone] then
				seen[from.zone] = true
				validzones[#validzones + 1] = from.zone
			end
			if checkValid(to) and not seen[to.zone] then
				seen[to.zone] = true
