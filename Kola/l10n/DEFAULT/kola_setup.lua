...
evc:addEvent({
	...
	action = function()
		...
		RegisterGroupTarget('evt-attack',250,'Intercept enemy airstrike','cas')
...
mc:trackMission({
   ...
   messageEnd =function() lastEscort_COOLDOWN=timer.getTime() return "Escort cargo plane ended: Escort cargo plane" end,
    startAction = function()
         if not missionCompleted and trigger.misc.getUserFlag(180) == 0 then
            trigger.action.outSoundForCoalition(2, "ding.ogg")
        end
...
evc:addEvent({
	...
	action = function()
		...
		RegisterGroupTarget('Zapolyarnyy-Scuds',100,'Destroy Scuds','scuds2')
		timer.scheduleFunction(function(param, time)
...
evc:addEvent({
    ...
    RespawnGroup('Kittila-Scuds')  
		RegisterGroupTarget('Kittila-Scuds',100,'Destroy Scuds','scuds1')
		timer.scheduleFunction(function(param, time)
...
evc:addEvent({
	...
	if not tgt then
		...
		RegisterScoreTarget('RussiansHideOut',tgt,500,'Russian hideout',true)
...
evc:addEvent({
	...
	if not tgt then
		...
		RegisterScoreTarget('GeneralsHouse',tgt,500,'High value target',true)
	local p = tgt:GetDCSObject() and tgt:GetDCSObject():getPoint()
		if p then
			missionMarkId = missionMarkId + 1
...
evc:addEvent({
	...
	action = function()
		...
		RegisterGroupTarget('PapasSon',500,'Right hand man','PapasSon',true)
	end,
	canExecute = function()
...
evc:addEvent({
	id = 'laplandiyaStorage',
		...
		RegisterStaticGroup('laplandiyaStorage', z, 1250, 'laplandiya Storage', 'laplandiyaStorage', true)
...
evc:addEvent({
	id = 'ZelenoborskijStorage',
		...
		if not z then return end
		RegisterStaticGroup('ZelenoborskijStorage', z, 1250, 'laplandiya Storage', 'ZelenoborskijStorage', true)
...
evc:addEvent({
	id = 'PyaozerskyStorage',
		...
		if not z then return end
		RegisterStaticGroup('PyaozerskyStorage', z, 1000, 'Pyaozersky Storage', 'PyaozerskyStorage', true)
...
evc:addEvent({
	id = 'lovozeroCCCBunker',
		...
		if not z then return end
		RegisterStaticGroup('lovozeroCCCBunker', z, 1000, 'lovozeroCCC Command Center', 'lovozeroCCCBunker', true)
